{{--
    Sesuaikan @extends() di bawah dengan nama layout dokter kamu
    kalau ternyata bukan 'layouts.dokter'.
--}}
...
<div class="p-6 md:p-10 space-y-6">
    ...
    <div class="bg-white rounded-2xl shadow-md border border-slate-100 p-6">
        ...
        <form method="POST" action="{{ route('dokter.complaint.store') }}" class="space-y-3">
            @csrf
        ...
        @forelse($complaints as $complaint)
            <div class="bg-white rounded-2xl shadow-md border border-slate-100 p-5">
                ...
                @if($complaint->status === 'resolved')
                    <form method="POST" action="{{ route('dokter.complaint.confirm', $complaint->id) }}" class="mt-4">
                        @csrf
                        @method('PUT')
                        <button type="submit"
