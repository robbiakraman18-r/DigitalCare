...
<div class="space-y-6">
    ...
    <div>
        ...
        <p class="text-slate-400 mt-1">
            Update the practice schedule for this doctor.
        </p>
    </div>
    ...
    <form
    action="{{ route('admin.doctor.schedule.update', $jadwal->id_jadwal) }}"
    method="POST"
    class="space-y-6">
    @csrf
    @method('PUT')
        ...
        <div class="grid grid-cols-1 sm:grid-cols-2 gap-5">
                ...
                <div class="relative mt-2">
                    ...
                    class="w-full pl-11 pr-10 py-3 rounded-2xl border border-slate-200 bg-white appearance-none focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-transparent transition">
                        ...
                        @foreach($dokters as $dokter)
                            @php
                                $isCurrentDoctor = $dokter->id_dokter == $jadwal->id_dokter;
                                $isUnavailable   = $dokter->status_ketersediaan !== 'Available';
                                $isInactive      = optional($dokter->user)->status !== 'active';
                                $isBlocked       = ($isUnavailable || $isInactive) && !$isCurrentDoctor;
                            ...
                            {{ old('id_dokter', $jadwal->id_dokter) == $dokter->id_dokter ? 'selected' : '' }}
                            {{ $isBlocked ? 'disabled' : '' }}>
                                {{ $label }}
                            </option>
                        @endforeach
            ...
            <div>
                ...
                <div class="relative mt-2">
                    ...
                    type="number"
                    name="kuota_harian"
                    value="{{ old('kuota_harian', $jadwal->kuota_harian) }}"
                    required
                    min="1"
                    placeholder="e.g. 20"
                    class="w-full pl-11 pr-4 py-3 rounded-2xl border border-slate-200 focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-transparent transition">
    ...
    <div class="flex justify-end gap-3">
        ...
        <button
        type="submit"
        class="px-6 py-3 rounded-2xl bg-cyan-600 hover:bg-cyan-700 text-white font-semibold transition inline-flex items-center gap-2">
