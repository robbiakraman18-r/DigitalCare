<!DOCTYPE html>
<html lang="id">
<head>
    <meta charset="UTF-8">
    <meta name="viewport" content="width=device-width, initial-scale=1.0">
    <title>DigitalCare</title>

    @vite(['resources/css/app.css','resources/js/app.js'])

    <link href="https://fonts.googleapis.com/css2?family=Inter:wght@400;500;600;700;800&display=swap" rel="stylesheet">

    <script src="https://unpkg.com/lucide@latest"></script>

    <script defer src="https://unpkg.com/alpinejs@3.x.x/dist/cdn.min.js"></script>
</head>

<body
    class="bg-gradient-to-br from-[#f0fffd] via-[#f8fffe] to-[#ecfffb] min-h-screen font-[Inter] flex overflow-x-hidden"
    x-data="{ openProfile: false, logoutModal: false }">

    <div class="fixed top-0 left-0 w-72 h-72 bg-teal-200/40 blur-3xl rounded-full -z-10"></div>
    <div class="fixed bottom-0 right-0 w-80 h-80 bg-cyan-200/40 blur-3xl rounded-full -z-10"></div>

    <x-sidebar-pasien />

    <div class="flex-1 ml-20">
        
        <x-topbar-pasien />
        
        <main class="w-full max-w-screen-2xl mx-auto px-6 xl:px-10 2xl:px-14 pb-10">
        @yield('content')
        </main>
        
    </div>

    <div
        x-show="logoutModal"
        x-transition
        class="fixed inset-0 bg-black/40 backdrop-blur-sm flex items-center justify-center z-50"
        style="display:none;"
        @click.away="logoutModal = false">

        <div class="bg-white rounded-3xl p-8 w-full max-w-sm" @click.stop>
            <div class="w-16 h-16 rounded-full bg-red-100 flex items-center justify-center mx-auto">
                <i data-lucide="log-out" class="text-red-500 w-8 h-8"></i>
            </div>

            <h2 class="text-2xl font-bold text-center mt-5">Logout?</h2>
            <p class="text-center text-slate-500 mt-2">Apakah Anda yakin ingin keluar?</p>

            <div class="grid grid-cols-2 gap-4 mt-8">
                <button
                    @click="logoutModal = false"
                    class="py-3 rounded-2xl border font-medium hover:bg-slate-50 transition">
                    Batal
                </button>

                {{-- FORM POST LOGOUT LARAVEL (Lebih aman dari sekadar tag <a> menuju /login) --}}
                <form action="{{ route('logout') }}" method="POST" class="inline">
                    @csrf
                    <button type="submit" class="w-full py-3 rounded-2xl bg-red-500 text-white text-center font-medium hover:bg-red-600 transition">
                        Logout
                    </button>
                </form>
            </div>
        </div>
    </div>
    
    <script>
        document.addEventListener('DOMContentLoaded', () => {
            lucide.createIcons();
        });
    </script>

    @stack('scripts')

</body>
</html>